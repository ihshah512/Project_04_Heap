...
public:
    ...
    Post()
    {
        m_postID = DEFAULTPOSTID;
        m_likes = MINLIKES;
        m_connectLevel = MAXCONLEVEL;
        m_postTime = MAXTIME;
    ...
    Post(int ID, int likes, int connectLevel, int postTime, int interestLevel)
    {
        ...
        if (likes < MINLIKES || likes > MAXLIKES)
            ...
            m_likes = likes;
        if (connectLevel < MINCONLEVEL || connectLevel > MAXCONLEVEL)
            m_connectLevel = MAXCONLEVEL;
        else
            m_connectLevel = connectLevel;
        if (postTime < MINTIME || postTime > MAXTIME)
            m_postTime = MAXTIME;
        else
            m_postTime = postTime;
        if (interestLevel < MININTERESTLEVEL || interestLevel > MAXINTERESTLEVEL)
            m_interestLevel = MININTERESTLEVEL;
        else
